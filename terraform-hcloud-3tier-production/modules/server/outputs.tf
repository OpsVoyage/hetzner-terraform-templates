output "servers" {
  ...
  value = {
    for k, s in hcloud_server.this : k => {
      ...
      ipv6        = s.ipv6_address
      status      = s.status
      private_ips = try([for n in hcloud_server.this[k].network : n.ip], [])
    }
  }
}
...
output "private_ips" {
  ...
  value       = flatten([for k in sort(keys(hcloud_server.this)) : try([for n in hcloud_server.this[k].network : n.ip], [])])
}
...
output "first_private_ip" {
  ...
  value       = try(one([for n in hcloud_server.this[keys(hcloud_server.this)[0]].network : n.ip]), null)
}
